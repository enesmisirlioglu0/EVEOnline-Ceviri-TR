...
> **Mevcut durum: Geliştirme aşaması 0.4f, tam ekran Space'teki gerçek EVE oyun penceresi güvenli biçimde seçildi.** Ekran Kaydı izni verildikten sonra hedefe özel `SCShareableContent` envanteri yalnız doğru EVE sürecini buldu. macOS tam ekran ana yüzeyi normal pencere katmanı yerine adlandırılmış durum pencere katmanında sunduğu için seçim politikası yalnız `normalWindow` ve `statusWindow` seviyelerini kabul edecek biçimde düzeltildi; arayüz canlı turda **İzin Hazır** ve **Seçildi** gösterdi. Boyutlar tek tek kodlanmaz: 320 × 180 güvenlik tabanının üzerindeki bütün geçerli EVE pencere çözünürlükleri adaydır; bu taban ince yardımcı şeritleri ana oyun yüzeyinden ayırmak amacıyla kullanılır. Ekran yakalama akışı, sol Command tetikleyicisi, OCR, çeviri ve oyun yanında gösterilecek panel henüz uygulanmadı. Aşama numarası uygulamanın pazarlama sürümü değildir.
...
Snapshot hedef sürece özeldir: yalnız istenen PID + paket kimliğine ait ekranda görünen pencere değerleri saklanır; başka uygulamaların pencere başlıkları veya görünen adları kopyalanmaz. Saf eşleştirici kendisine verilen EVE süreç descriptor'ındaki paket/çalıştırılabilir kimliği ile öndelik durumunu, ScreenCaptureKit tarafında da owner PID + paket kimliğini birlikte denetler. Yalnız adlandırılmış normal ve durum pencere katmanlarını kabul eder; sahipsiz, görünmeyen, bozuk geometrili veya oyun yüzeyi olamayacak kadar küçük pencereleri eler ve yakın eşitlikte rastgele seçim yapmak yerine belirsizlik üretir. Çözünürlük listesi kullanılmaz: 320 × 180 tabanını geçen bütün pencere boyutları aynı kurallarla değerlendirilir. Önceki pencere yalnız aynı, `launchDate` ile ayırt edilebilen süreç neslinde ve hâlâ yeterince büyük/görünürse korunur. `launchDate` yoksa eski seçim tekrar kullanılmaz.
...
Canlı Launcher-only turunda arayüz **Launcher Açık** durumunu ve “Launcher açık; EVE oyun istemcisi bekleniyor” açıklamasını gösterdi. Daha sonraki açık izinli turda EVE tam ekran Space'te öne getirildi; hedefe özel ScreenCaptureKit envanterindeki büyük ana yüzey, ince yardımcı yüzey elenirken seçildi ve arayüz **Seçildi** durumuna geçti. Bu tur yalnız pencere metadata seçimini kanıtlar: piksel akışı başlatılmadı. Seçim başarılı olsa da mevcut geometrik sunum sınıflayıcısı sonucu `.normal` olarak sunduğundan `.fullScreen` etiketi henüz doğrulanmış sayılmaz; bu ayrım 0.5'te canlı akış ve koordinat testleriyle tamamlanacaktır.
